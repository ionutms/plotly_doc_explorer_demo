...
def create_labeled_range_slider_column(
    slider_id: dict, label_text: str, min_val: int = 0, pushable_val: int = 0
) -> dbc.Col:
    """
...
def create_three_level_filter_row(instance_id: int = 0) -> dbc.Row:
    ...
    """
    level_sliders_column = dbc.Col(
        [
            dbc.Row([
                create_labeled_range_slider_column(
                    {"type": "slider_1", "index": instance_id},
                    "Level 1 items",
                    1,
                    1,
                ),
                create_labeled_range_slider_column(
                    {"type": "slider_2", "index": instance_id},
                    "Level 2 items",
                ),
                create_labeled_range_slider_column(
                    {"type": "slider_3", "index": instance_id},
                    "Level 3 items",
                ),
            ])
        ],
    ...
    three_level_filter_row = dbc.Row([
        dbc.Col([dbc.Row([level_sliders_column, switch_column])])
    ])
...
def create_main_controls_accordion(index_id: int = 0):
    ...
    """
    main_controls_accordion = dbc.Row([
        dbc.Col([
            dcc.Store(
...
def create_graph_and_iframe_section(index_id: int = 0):
    ...
    """
    graph_section_column = dbc.Col(
        [
            dcc.Loading([
                dcc.Graph(
                    id={"type": "treemap", "index": index_id},
                    config=fig_u.GRAPH_CONFIG,
                    style={"display": "none"},
                )
            ])
        ],
    ...
    iframe_section_column = dbc.Col(
        [
            dbc.Row([
                dbc.Col([
                    html.A(id={"type": "click_data", "index": index_id})
                ])
            ]),
            html.Iframe(
...
def reset_store_count_on_checklist_change(
    checklist: List[str], data: Dict[str, Any]
) -> Dict[str, Any]:
    """
...
@callback(Output("container", "children"), Input("store", "data"))
...
def update_treemap_and_store(
    ...
    theme = {
        ...
        "title_text": None,
        "uniformtext": {"minsize": 16, "mode": False},
    }
    treee_fig.update_layout(**theme)

    treee_fig.add_trace(
        go.Treemap(marker={"cornerradius": 5}, maxdepth=4, sort=sort_switch)
    )
...
def update_treemap_based_on_slider_inputs(
    slider_1_value: List[int],
    slider_2_value: List[int],
...
def update_click_data_display(
    ...
    iframe_style["display"] = (
        "" if web_u.check_section_exists(doc_url) else "none"
    )
