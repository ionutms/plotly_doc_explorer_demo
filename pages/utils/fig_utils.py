...
def find_last_options(
    ...
    zip_diff = [
        item
        for item in sorted(
            list(set(zip(tree["parents"], tree["labels"], tree["ids"])))
        )
    ...
    for parent, label, zip_id in zip_diff:
        ...
        try:
            ...
            if not isinstance(root_go_obj[parent][label], (str, tuple)):
                new_labels = [
                    key
                    for key in sorted(list(set(root_go_obj[parent][label])))
                ...
                if split is not None:
                    try:
                        new_labels = new_labels[
                            split["level_3"]["start"] : split["level_3"][
                                "end"
                            ]
                        ]
                    except KeyError:
...
def find_mid_options(
    ...
    for label in tree["labels"]:
        try:
            ...
            if not isinstance(root_go_obj[label], (str, tuple)):
                options[label] = [
                    key
                    for key in sorted(set(root_go_obj[label]))
                    if not key.endswith("src")
    ...
    for new_key, new_options in options.items():
        ...
        tree["parents"].extend(
            [f"{root_key}*{new_key}" if root_key != "" else f"{new_key}"]
            * len(new_options)
        )
        ...
        tree["ids"].extend([
            f"{root_key}*{new_key}*{option}"
            if root_key != ""
            else f"{new_key}*{option}"
...
def find_first_options(
    ...
    if split is not None:
        try:
            zipped = zipped[
                split["level_1"]["start"] : split["level_1"]["end"]
            ]
        except KeyError:
            pass
...
GO_INFO = {
    ...
    "Geo": create_go_info_item(go.layout.Geo, "layout/geo", "layout-geo"),
